...
function looper(family) {
  family.forEach(member => alert(member));
}
...
function divider(numbersArray) {
  return [
    ...
    numbersArray.filter(num => (num % 2) - 1 == 0)
  ];
}
...
function longer(arr1, arr2) {
  return arr1.length > arr1.length ? arr1 : arr2;
}
...
devMountainEmployees = devMountainEmployees.filter(employee => {
